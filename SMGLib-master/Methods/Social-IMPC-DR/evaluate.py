...
def evaluateMetrics():
    ...
    # Check if the file is a CSV and starts with 'cs'
        if filename.startswith('path_deviation_robot_') and filename.endswith('.csv'):

            uav_num = filename[21:-4]
            ...
            print("*" * 65)
            print("Path Deviation of robot " + uav_num + ":")
            print(f"L2 Norm: {l2_norm.sum():.4f}")
            print(f"Hausdorff distance: {hausdorff_dist:.4f}")
            print("*" * 65)

        if filename.startswith('avg_delta_velocity_robot_') and filename.endswith('.csv'):
            uav_num = filename[25:-4]
            # Construct the full file path
            file_path = os.path.join(directory, filename)
            ...
            print("*" * 65)
            print("Avg delta velocity for robot " + uav_num + ": " +  f"{sum_abs_diffs:.4f}")
            print("*" * 65)
...
def main():
    ...
    parser.add_argument("--evaluateMetrics", type=str, help="")
    

    args = parser.parse_args()

    if args.evaluateMetrics:
        evaluateMetrics()
